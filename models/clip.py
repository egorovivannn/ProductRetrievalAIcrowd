...
#                                           features_only = True
        ...
        in_features = getattr(self.backbone, self.backbone.default_cfg['classifier']).in_features
        setattr(self.backbone, self.backbone.default_cfg['classifier'], torch.nn.Identity())
        

        if cfg.pool == "gem":
            self.global_pool = GeM1d(p_trainable=cfg.gem_p_trainable)
        elif cfg.pool == "identity":
            self.global_pool = torch.nn.Identity()
        elif cfg.pool == "avg":
            self.global_pool = nn.AdaptiveAvgPool1d(512)

        self.embedding_size = cfg.embedding_size

        self.neck = nn.Sequential(
                nn.Linear(768, self.embedding_size, bias=True),
